...
export default function CartDrawer() {
  ...
  const { products, convertirPrix, symboleDevise, t } = useStore();
      ...
      <div className="fixed inset-0 z-50 overflow-hidden">
        ...
        <div className="fixed inset-y-0 right-0 max-w-md w-full bg-[#FAF7F5] shadow-2xl flex flex-col justify-between text-[#2C2224] border-l border-white/60">
          <div className="px-6 py-5 flex items-center justify-between border-b border-[#E88D9E]/15 bg-white/60 backdrop-blur-md sticky top-0 z-10">
            <div><span className="text-[9px] font-mono tracking-[0.25em] text-[#E88D9E] uppercase font-bold">{t("cart.label").toUpperCase()}</span><h2 className="text-xl font-serif font-bold">{t("cart.title")}</h2></div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {items.length === 0 ? <p className="text-center py-20 text-gray-400 font-mono text-xs uppercase tracking-widest">{t("cart.empty")}</p> : (
              <>
